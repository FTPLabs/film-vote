  ...
  return (
    <div className="max-w-4xl mx-auto space-y-12 pb-16">
      ...
      >
        ...
        <h1 className="text-4xl md:text-5xl font-display font-bold">🏆 Топ рейтинга</h1>
        <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
          Что деревня хочет смотреть больше всего — по голосам «За».
        </p>
      </motion.div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <motion.div
          ...
          className="bg-card border rounded-2xl p-6 flex items-center gap-4 shadow-sm"
        >
        ...
        <div className="space-y-4">
          ...
          {stats.topMovies.length === 0 && (
            <motion.div
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="text-center py-12 bg-secondary/50 rounded-2xl border border-dashed border-border"
            >
              <Trophy className="w-12 h-12 text-muted-foreground mx-auto mb-3" />
              <p className="text-muted-foreground font-medium">Рейтинг пока формируется. Проголосуйте первым! 🗳️</p>
            </motion.div>
          )}
        </div>
      </div>
